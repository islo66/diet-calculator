...
<div class="grid grid-cols-12 gap-4">
    ...
    <div class="col-span-6 md:col-span-4">
        <label class="block text-sm font-medium text-gray-700 mb-1">Unitate Default *</label>
        <select
            name="default_unit"
            class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
            required
        >
            <option value="g" {{ old('default_unit', $food->default_unit ?? 'g') === 'g' ? 'selected' : '' }}>g (grame)</option>
            <option value="ml" {{ old('default_unit', $food->default_unit ?? '') === 'ml' ? 'selected' : '' }}>ml (mililitri)</option>
            <option value="pcs" {{ old('default_unit', $food->default_unit ?? '') === 'pcs' ? 'selected' : '' }}>pcs (bucati)</option>
        </select>
    </div>
    ...
    <div class="col-span-6 md:col-span-3">
        <label class="block text-sm font-medium text-gray-700 mb-1">Densitate (g/ml)</label>
        <input
            type="number"
            ...
            min="0.001"
        >
        <p class="mt-1 text-xs text-gray-500">Pentru conversii g ↔ ml</p>
    </div>
